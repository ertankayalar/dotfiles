...
keys = [
    ...
    Key(
        [mod, "shift"], "s", lazy.spawn("kitty -e bettercd.sh"), desc="my file manager"
    ),
    # =-/ Browsers /-=#
...
separator = widget.Sep(
    size_percent=50, foreground=colors["fg3"], linewidth=1, padding=10
)
...
def init_widgets_list():
    widgets_list = [
        ...
        widget.GroupBox(
            ...
            other_screen_border=colors["dark-cyan"],
        ...
        widget.CurrentLayout(
            foreground=colors["fg1"], background=colors["bg2"], padding=5
        ),
...
def init_screens():
    return [
        Screen(
            top=bar.Bar(widgets=init_widgets_screen1(), opacity=1.0, size=33, margin=3),
